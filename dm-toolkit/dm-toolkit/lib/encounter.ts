...
export function calculateEncounterDifficulty(
  ...
  const thresholds = xpThresholds[partyLevel as keyof typeof xpThresholds] || xpThresholds[1]
...
export function getEncounterMultiplier(monsterCount: number, partySize: number): number {
  ...
  if (partySize < 3) {
    adjustedCount = Math.ceil(monsterCount * 1.5)
  } else if (partySize > 5) {
    adjustedCount = Math.floor(monsterCount * 0.5)
  }
  
  const multiplierEntry = encounterMultipliers.find(
    entry => adjustedCount >= entry.min && adjustedCount <= entry.max
  )
  
  return multiplierEntry?.multiplier || 1
}
...
export function getDifficultyLabel(
  ...
  if (totalXP >= difficulty.deadly) return 'Deadly'
  if (totalXP >= difficulty.hard) return 'Hard'
  if (totalXP >= difficulty.medium) return 'Medium'
  if (totalXP >= difficulty.easy) return 'Easy'
  return 'Trivial'
}